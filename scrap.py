...
def create_json_file(dictContent ,strPath):
    ...
    if strPath == '':
        with open(f'photos.json', "w+",encoding='utf-8') as file:
        ...
        with open(f'{strPath}.json', 'w', encoding='utf-8') as file:
            json.dump(dictContent, file, indent=4, ensure_ascii=False)
    

def start(strText):
    strText_full = 'https://yandex.ru/images/search?from=tabbar&text=' + strText
    # print(strText)

    listPhotoObjs = get_Photos_objs(strText_full)
    dictPhotoObjs = dictFromList(listPhotoObjs)
    create_json_file(dictPhotoObjs,strText)